...
const SKILL_RELATIVE_DIR = path.join('.claude', 'skills', 'graphql-schema');
...
function findRepoRoot(start = process.cwd()) {
  ...
  while (true) {
    if (
      ...
      fs.existsSync(path.join(current, SKILL_RELATIVE_DIR))
    ) {
      return current;
    }
...
const ENV_PATH = process.env.ARMS_ENV_PATH || path.join(REPO_ROOT, '.env');
const SCHEMA_DIR =
  process.env.ARMS_SCHEMA_DIR ||
  (fs.existsSync(path.join(REPO_ROOT, SKILL_RELATIVE_DIR))
    ? path.join(REPO_ROOT, SKILL_RELATIVE_DIR, 'schemas')
    : path.join(REPO_ROOT, 'schemas'));
const COMMAND_NAME = path.basename(process.argv[1] || 'schema');
...
function loadSchema(service) {
  ...
  if (!fs.existsSync(p)) {
    die(
      `No schema for '${service}' at ${p}. Run: ${COMMAND_NAME} update ${service}`
    );
  }
...
function usage() {
  process.stdout.write(
    [
      'Usage:',
      `  ${COMMAND_NAME} list <service> [queries|mutations|subscriptions]`,
      `  ${COMMAND_NAME} get <service> <name>`,
      `  ${COMMAND_NAME} search <service> <keyword>`,
      `  ${COMMAND_NAME} update <service|all> [--token <jwt>]`,
      '',
